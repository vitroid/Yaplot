...
Winfo *GetOptions(int argc, char *argv[], Ainfo *a, Ginfo *g)
{
	...
	if (errflg) {
		...
		fprintf(stderr,
			"\t-q n    \tset cache size to n frames (default=1)\n");
		fprintf(stderr, "\t-c filename\tspecify palette file(default: "
				"yaplot.col)\n");
		fprintf(stderr, "\t If no palette file is found, envvar "
				"YAPLOTCOL is checked,\n");
	...
	for (i = 0; i < g->nwindow; i++) {
		if (strcmp(w[i].inputfilename, "-") == 0) {
			...
			if (nstdin > 1) {
				fprintf(stderr,
					"%s : Two \"-\"s are specified at a "
					"time.\n",
					g->commandname);
				exit(-1);
			}
		} else if ((w[i].inputfile = fopen(w[i].inputfilename, "r")) ==
			   NULL) {
...
int ePushView(Ginfo *g, Winfo w[], int i)
{
	...
	sprintf(buf, "%f %f %f %f %f %f %f %f %f %f %f\n", w[i].fov,
		w[i].depthratio, w[i].eyep[0], w[i].eyep[1], w[i].eyep[2],
		w[i].lookp[0], w[i].lookp[1], w[i].lookp[2], w[i].up[0],
		w[i].up[1], w[i].up[2]);
	fputs(buf, file);
...
int ePopView(Ginfo *g, Winfo w[], int i)
{
	...
	if (pos1 >= 0) {
		sscanf(buf, "%f %f %f %f %f %f %f %f %f %f %f\n", &w[i].fov,
		       &w[i].depthratio, &w[i].eyep[0], &w[i].eyep[1],
		       &w[i].eyep[2], &w[i].lookp[0], &w[i].lookp[1],
		       &w[i].lookp[2], &w[i].up[0], &w[i].up[1], &w[i].up[2]);
		W_setvectors(&w[i]);
		w[i].status |= REDRAW;
		return TRUE;
	}
